...
func Test_defaultClient_RegisterAndDispatchEventListener(t *testing.T) {
	t.Parallel()
	...
	listener := &EventListener{
		ID: "listener-1",
		Filter: func(msg *EventMessage) bool {
			if msg == nil {
				return false
			}
			return msg.Data.Event.Metadata.Path == "expected/path"
		},
		Callback: func(ctx context.Context, msg *EventMessage) error {
			callCount++
			return nil
		},
	}

	// registering nil or incomplete listeners should be a no-op
	require.NotPanics(t, func() {
		c.RegisterEventListener(nil)
		c.RegisterEventListener(&EventListener{})
	})

	c.RegisterEventListener(listener)
	require.Len(t, c.eventListeners, 1)

	// dispatch with a matching event should invoke the callback
	msg := &EventMessage{}
	msg.Data.Event.Metadata.Path = "expected/path"
	c.dispatchEvent(context.Background(), msg)
	assert.Equal(t, 1, callCount)

	// dispatch with a non-matching event should not invoke the callback
	nonMatching := &EventMessage{}
	nonMatching.Data.Event.Metadata.Path = "other/path"
	c.dispatchEvent(context.Background(), nonMatching)
	assert.Equal(t, 1, callCount)

	// dispatch with a nil message should be ignored
	c.dispatchEvent(context.Background(), nil)
	assert.Equal(t, 1, callCount)
}

func Test_defaultClient_UnregisterEventListener(t *testing.T) {
	t.Parallel()
	...
	// Unregister should remove the listener, and subsequent dispatches should not call it.
	c.UnregisterEventListener("listener-2")
	require.Len(t, c.eventListeners, 0)

	msg := &EventMessage{}
	c.dispatchEvent(context.Background(), msg)
	assert.False(t, called, "callback should not be invoked after UnregisterEventListener")

	// Unregister of unknown IDs should be a no-op
	require.NotPanics(t, func() {
		c.UnregisterEventListener("missing")
	})
}
