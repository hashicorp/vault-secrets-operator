...
@test "controller/Deployment: default resources for controller and job" {
  ...
  local actual
  actual=$(echo "$controller" | yq '.requests.cpu' | tee /dev/stderr)
  [ "${actual}" = "10m" ]
...
@test "controller/Deployment: can set resources for controller" {
  ...
  local controller
  controller=$(echo "$object" | yq '.spec.template.spec.containers[] | select(.name == "manager") | .resources' | tee /dev/stderr)
  local jobs
  job=$(echo "$object" | yq '.spec.template.spec.containers[] | select(.name == "pre-delete-controller-cleanup") | .resources' | tee /dev/stderr)
...
@test "controller/Deployment: clientCache not set by default" {
  ...
  local actual
  actual=$(echo "$object" | yq 'contains(["--client-cache"])' | tee /dev/stderr)
  [ "${actual}" = "false" ]
}
...
@test "controller/Deployment: clientCache settings can be set" {
  ...
  local actual
  actual=$(echo "$object" | yq 'contains(["--client-cache-size=22", "--client-cache-persistence-model=direct-encrypted"])' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}
...
@test "controller/Deployment: maxConcurrentReconciles not set by default" {
  ...
  local actual
  actual=$(echo "$object" | yq 'contains(["--max-concurrent-reconciles"])' | tee /dev/stderr)
  [ "${actual}" = "false" ]
}
...
@test "controller/Deployment: maxConcurrentReconciles can be set" {
  ...
  local actual
  actual=$(echo "$object" | yq 'contains(["--max-concurrent-reconciles=5"])' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}
...
@test "controller/Deployment: controller.podSecurityContext set by default" {
  ...
  local actual
  actual=$(echo "$object" | yq 'select(documentIndex == 1) | length' | tee /dev/stderr)
  [ "${actual}" = "1" ]
  actual=$(echo "$object" | yq 'select(documentIndex == 1) | .runAsNonRoot' | tee /dev/stderr)
  [ "${actual}" = "true" ]

  actual=$(echo "$object" | yq 'select(documentIndex == 2) | length' | tee /dev/stderr)
  [ "${actual}" = "1" ]
...
@test "controller/Deployment: without strategy" {
  ...
  local actual
  actual=$(echo "$object" | yq '.strategy' | tee /dev/stderr)
  [ "${actual}" = "null" ]
}
...
@test "controller/Deployment: with rollingUpdate strategy" {
  ...
  local actual
  actual=$(echo "$object" | yq '.strategy | length' | tee /dev/stderr)
  [ "${actual}" = "2" ]
  actual=$(echo "$object" | yq '.strategy.type' | tee /dev/stderr)
  [ "${actual}" = "rollingUpdate" ]
  actual=$(echo "$object" | yq '.strategy.rollingUpdate | length' | tee /dev/stderr)
  [ "${actual}" = "2" ]
