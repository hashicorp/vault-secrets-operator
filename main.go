...
func main() {
	...
	flag.StringVar(&metricsAddr, "metrics-bind-address", ":8080",
		"The address the metric endpoint binds to. "+
			"Use :8443 for HTTPS with authn/authz protection, or 0 to disable the metrics service.")
	flag.BoolVar(&secureMetrics, "metrics-secure", false,
		"If set, the metrics endpoint is served securely via HTTPS and protected by "+
			"authentication and authorization (SubjectAccessReview/TokenReview). "+
			"Use --metrics-secure=false to serve metrics via HTTP without protection.")
	...
	// Disable HTTP/2 by default to mitigate the HTTP/2 Stream Cancellation and
	...
	// Metrics endpoint options. When secureMetrics is enabled the endpoint is
	// served via HTTPS and protected with authn/authz, replacing the
	// kube-rbac-proxy sidecar. Certificates are generated by controller-runtime
	...
	if secureMetrics {
		// FilterProvider protects the metrics endpoint with authn/authz. The
		// RBAC required for this is configured in config/rbac and the Helm chart.
