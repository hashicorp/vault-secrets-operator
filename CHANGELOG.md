...
BREAKING CHANGES:
* Remove the `kube-rbac-proxy` sidecar. The `/metrics` endpoint is now served directly by the operator over HTTPS on port `8443` and protected in-process by controller-runtime's `filters.WithAuthenticationAndAuthorization`, which performs the same `TokenReview`/`SubjectAccessReview` checks the sidecar previously did. This removes the dependency on the `gcr.io/kubebuilder/kube-rbac-proxy` image, which became unavailable on March 18, 2025. The `controller.kubeRbacProxy` Helm values (`image.repository`, `image.tag`, `image.pullPolicy`, `resources`) have been **removed** — any values file setting them must be updated, as `helm upgrade` will fail on unknown values only if `--strict` is used, otherwise they are silently ignored. The metrics Service, its port (`8443`), and the ServiceMonitor configuration are unchanged, so Prometheus scrape configuration using a bearer token continues to work. Note that unlike `kube-rbac-proxy`, the in-process filter authenticates via bearer tokens only and does not support client-certificate (mTLS) authentication.
