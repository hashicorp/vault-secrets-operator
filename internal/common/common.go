...
func MergeInVaultAuthGlobal(ctx context.Context, c ctrlclient.Client, o *secretsv1beta1.VaultAuth) (*secretsv1beta1.VaultAuth, *secretsv1beta1.VaultAuthGlobal, error) {
	...
	switch s := cObj.Spec.VaultAuthGlobalRef.MergeStrategy.Params; s {
	case "union":
	...
	case "none":
	default:
		return nil, nil, fmt.Errorf("unsupported params merge strategy %q", s)
	}

	switch s := cObj.Spec.VaultAuthGlobalRef.MergeStrategy.Headers; s {
	case "union":
	...
	case "none":
	default:
		return nil, nil, fmt.Errorf("unsupported headers merge strategy %q", s)
	}
