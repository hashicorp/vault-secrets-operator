...
| `secretIDPath` _string_ | SecretIDPath is a file system path pointing to a file containing the plaintext Secret ID for the AppRole Role to use for authenticating to Vault.<br />If both SecretIDPath and SecretRef are specified, SecretIDPath takes precedence. |  |  |
